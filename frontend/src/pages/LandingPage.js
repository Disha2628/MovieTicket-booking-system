...
const LandingPage = () => {
  ...
  const [heroMovies, setHeroMovies] = useState([
    {
      id: 1,
      title: 'Interstellar',
      poster: '/Movies_posters/Interstellar_poster.jpeg',
  ...
  const fetchMovies = async () => {
    try {
      ...
      setMovies(
        Array.isArray(response.data) ? response.data : response.data.movies || []
      );
    } catch (error) {
  ...
  const selectOption = (key, value) => {
    if (key === 'allMovies') setGenreFilter(value === 'All Movies' ? '' : value);
      ...
      setLanguageFilter(value === 'by Category' ? '' : value);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white flex flex-col items-center p-4 relative overflow-hidden">
      ...
      <div className="w-full min-h-[70vh] relative overflow-hidden rounded-3xl shadow-2xl shadow-black/50 mx-auto max-w-7xl my-5 z-10">
        <div className="flex w-full h-full overflow-x-auto scroll-smooth" ref={heroWrapperRef}>
          {Array.isArray(heroMovies) &&
            heroMovies.map((movie, index) => (
              <div
                key={movie.id}
                className="relative flex-shrink-0 w-full min-h-[70vh] flex items-center justify-start"
              >
                <div className="absolute inset-0 -z-20">
                  <img
                    src={`/banner${index + 1}.png`}
                    alt="bg"
                    className="w-full h-full object-cover brightness-80 blur-2xs"
                  />
                ...
                <div className="mt-6 ml-12 flex gap-16 items-center justify-start flex-wrap max-w-5xl">
                  <div className="relative">
                    <img
                      src={movie.poster}
                      alt={movie.title}
                      className="w-80 h-[28rem] rounded-2xl shadow-2xl shadow-blue-900/50 transition-transform duration-300 hover:scale-105"
                    />
                  </div>

                  <div className="max-w-md text-white">
                    <h1 className="text-6xl font-extrabold bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent inline-block">
                      {movie.title}
                    </h1>
                    ...
                    <div className="mt-6 flex gap-4">
                      <button className="bg-transparent border-2 border-gray-500 py-3 px-6 rounded-full text-gray-300 text-lg cursor-pointer transition-all duration-300 hover:bg-gray-700/50 hover:border-white">
                        Coming Soon...
                      </button>
      ...
      <div className="flex justify-center items-center gap-16 mx-auto max-w-6xl my-5 flex-wrap relative z-20 bg-gray-900/80 backdrop-blur-md rounded-full p-3 border border-gray-700 shadow-xl shadow-black/50">
        <div
          className="relative cursor-pointer text-gray-300 text-lg flex items-center group"
          onClick={() => toggleDropdown('allMovies')}
        >
          <span className="transition-all duration-300 group-hover:text-blue-400">
            {genreFilter || 'Current Movies'}
          </span>
          ...
          {dropdownOpen.allMovies && (
            <div className="absolute top-full mt-2 left-0 bg-gray-800 border border-blue-600/50 rounded-lg min-w-48 z-30 flex flex-col overflow-hidden shadow-2xl">
              <div
                className="px-4 py-2 text-gray-300 cursor-pointer transition-colors duration-200 hover:bg-blue-600/20 hover:text-white"
                onClick={() => selectOption('allMovies', 'All Movies')}
              >
                Current Movies
              </div>
              {genres.map((genre) => (
                <div
                  key={genre}
                  className="px-4 py-2 text-gray-300 cursor-pointer transition-colors duration-200 hover:bg-blue-600/20 hover:text-white"
                  onClick={() => selectOption('allMovies', genre)}
                >
                  {genre}
                </div>
              ))}
        ...
        <div
          className="relative cursor-pointer text-gray-300 text-lg flex items-center group"
          onClick={() => toggleDropdown('byCategory')}
        >
          <span className="transition-all duration-300 group-hover:text-blue-400">
          ...
          {dropdownOpen.byCategory && (
            <div className="absolute top-full mt-2 left-0 bg-gray-800 border border-blue-600/50 rounded-lg min-w-48 z-30 flex flex-col overflow-hidden shadow-2xl">
              <div
                className="px-4 py-2 text-gray-300 cursor-pointer transition-colors duration-200 hover:bg-blue-600/20 hover:text-white"
                onClick={() => selectOption('byCategory', 'by Language')}
              >
                All Languages
              </div>
              {languages.map((language) => (
                <div
                  key={language}
                  className="px-4 py-2 text-gray-300 cursor-pointer transition-colors duration-200 hover:bg-blue-600/20 hover:text-white"
                  onClick={() => selectOption('byCategory', language)}
                >
                  {language}
                </div>
              ))}
        ...
        <div
          className="cursor-pointer text-gray-300 text-lg flex items-center group"
          onClick={() => setComingSoon(!comingSoon)}
        >
          <span className="transition-all duration-300 group-hover:text-blue-400">
            {comingSoon ? 'All Movies' : 'Coming Soon'}
          </span>
        </div>
      ...
      <h2 className="text-center text-white-400 text-4xl my-8 font-extrabold tracking-wide drop-shadow-lg">
        {comingSoon ? 'Coming Soon...' : 'Available for booking...'}
      </h2>
      ...
      {trailerModalOpen && (
        <div
          className="fixed inset-0 bg-black/90 flex justify-center items-center z-50 backdrop-blur-md"
          onClick={closeTrailerModal}
        >
          <div
            className="relative w-11/12 max-w-4xl h-3/4 max-h-[500px] bg-gray-900 rounded-2xl overflow-hidden shadow-2xl border-2 border-blue-700/50"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              className="absolute top-4 right-4 bg-gray-800/70 text-white border-none rounded-full w-10 h-10 cursor-pointer text-xl z-10 transition-all duration-300 hover:bg-blue-600"
              onClick={closeTrailerModal}
            >
              ×
            </button>
            <iframe
              ...
              src={
                currentTrailerUrl
                  ? `https://www.youtube.com/embed/${currentTrailerUrl.split('v=')[1]}`
                  : ''
              }
              title="Movie Trailer"
              frameBorder="0"
